...
def plot_confusion_matrix(cm, my_tags, title='Confusion matrix', cmap=plt.cm.Blues, normalize=True):
    """
...
def evaluate_prediction(predictions, target, tags, title="Confusion matrix"):
    ...
    print(classification_report(target,predictions,tags))
...
def most_influential_words(model, vectorizer, genre_index=0, num_words=10):
    ...
    return [[features[x[0]], x[1] ] for x in max_coef[:num_words]], model.coef_[genre_index]
